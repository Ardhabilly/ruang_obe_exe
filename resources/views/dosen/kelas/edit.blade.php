    ...
    <div class="px-4 py-10 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-3xl">
            <div class="rounded-[2rem] border border-white/10 bg-white/[0.06] p-8 backdrop-blur-xl">
                <p class="text-sm font-semibold text-cyan-200">Edit Kelas</p>
                <h1 class="mt-2 text-3xl font-black text-white">{{ $classGroup->name }}</h1>

                <form action="{{ route('dosen.kelas.update', $classGroup) }}" method="POST" class="mt-8 space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $classGroup->name) }}"
                               class="mt-2 w-full rounded-2xl border-white/10 bg-slate-950/40 text-white focus:border-cyan-400 focus:ring-cyan-400">
                        @error('name')
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                                  class="mt-2 w-full rounded-2xl border-white/10 bg-slate-950/40 text-white focus:border-cyan-400 focus:ring-cyan-400">{{ old('description', $classGroup->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-300">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="kkm" value="{{ old('kkm', $classGroup->kkm) }}" min="0" max="100"
                               class="mt-2 w-full rounded-2xl border-white/10 bg-slate-950/40 text-white focus:border-cyan-400 focus:ring-cyan-400">
                        @error('kkm')
                            <p class="mt-2 text-sm text-red-300">{{ $message }}</p>
                        @enderror
                    ...
                    <label class="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $classGroup->is_active))
